...
static bool parse_notify(struct pool *pool, json_t *val)
{
    ...
    if(json_array_size(val) == 4)
        pool->has_header = true;
    else
        pool->has_header = false;
    ...
    if(!pool->has_header)
    {
        ...
        if (opt_protocol)
        {
            ...
            applog(LOG_NOTICE, "bbversion: %s",  pool->swork.bbversion);
	...
	else
	{
		...
		header = json_array_string(val, i++);
		target  =json_array_string(val, i++);
		clean = json_is_true(json_array_get(val, i));
		
		if (!job_id || !header || !target)
        {
            /* Annoying but we must not leak memory */
            if (job_id)
                free(job_id);
		...
		if(strstr(header,"0x") != NULL)
			snprintf(header,strlen(header)-1,"%s",header+2);
		if(strstr(target,"0x") != NULL)
			snprintf(target,strlen(target)-1,"%s",target+2);
    	...
    	swab256(pool->target, hash_swap);		
        pool->swork.clean = clean;
		
		if (clean)
            pool->nonce2 = 0;
		cg_wunlock(&pool->data_lock);
		if (unlikely(!hex2bin(pool->header_bin, header, 80)))
        {
            applog(LOG_WARNING, "%s: Failed to convert header to header_bin, got %s", __func__, header);
            pool_failed(pool);
			free(header);
        ...
        {
            applog(LOG_NOTICE, "job_id: %s", job_id);
            applog(LOG_NOTICE, "header: %s", header);
            applog(LOG_NOTICE, "target: %s", target);
    ...
    //printf("pool->swork.clean: %s \n", pool->swork.clean ? "yes" : "no");
    //printf("in function parse_notify before return we set ret = true and out \n");
