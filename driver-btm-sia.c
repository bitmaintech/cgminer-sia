...
void check_asic_reg(unsigned int which_chain, unsigned char mode, unsigned char chip_addr, unsigned char reg_addr)
{
    ...
    switch(reg_addr)
    {
        case CHIP_ADDR:
            ...
            if(update_asic_num[which_chain])
            {
                update_asic_num[which_chain] = false;
...
void chain_inactive(int const which_chain)
{
    ...
    sia_write(fd, cmd_buf, CMD_LENTH);
}
...
int bitmain_sia_init(struct bitmain_sia_info *info)
{
...
#ifdef UP_FREQ_MODE
    sleep(1);
    every_chain_set_voltage_PIC16F1704_new(820);
    every_chain_save_freq_PIC16F1704_new(600);
    sleep(1);
#endif
    ...
    if(config_parameter.timeout_eft)
    {
        ...
        dev.timeout = 0xffffffff / (0x100 / dev.addrInterval ) / dev.frequency / 128 * 0.75 * 13;
    ...
    while(nonce_fifo.nonce_num)
    {
        ...
        if(work)
        {
            ...
            if(submitnonceok)
            {
                ...
                which_asic_nonce = (nonce & 0x000000FF) / dev.addrInterval;
...
void *check_miner_status(void *arg)
{
	...
	bool need_reinit[BITMAIN_MAX_CHAIN_NUM] = {false};
	int ret,reinit_times[BITMAIN_MAX_CHAIN_NUM] = {0};
    ...
    while(1)
    {
            ...
            for(which_chain=0; which_chain<BITMAIN_MAX_CHAIN_NUM; which_chain++)
            {
                if(dev.chain_exist[which_chain])
                {
                    ...
                    if ( ( which_asic + offset ) > (ASIC_NUM_EACH_CHAIN + 16) )
                        applog(LOG_ERR, "asic num err![%d]", (which_asic + offset));
                    dev.chain_asic_status_string[which_chain][which_asic+offset] = '\0';
                    if((need_reinit[which_chain] || (each_chain_h_avg[which_chain]/1000000/10) < (double)(( ASIC_NUM_EACH_CHAIN * dev.frequency * dev.corenum * 0.80) / 13 ))&& (!status_error))
                    {
                    	reinit_times[which_chain]++;
						...
						ret = pthread_create(&reinit_id[which_chain], NULL, bitmain_sia_reinit_chain, (void *)(&info_with_index[which_chain]));
#ifdef REINIT_LOG
                        if(ret != 0)
							applog(LOG_NOTICE,"chain %d reinit fail errorno:%d message:%s",which_chain,ret,strerror(ret));
#endif
                        gMinerStatus_Low_Hashrate = true;
						need_reinit[which_chain] = false;
                    }
                    else
                    {
                    	reinit_times[which_chain] = 0;
                        gMinerStatus_Low_Hashrate = false;
                    }
#ifdef REINIT_ERR_REBOOT
					if(reinit_times[which_chain] >= 2)
					{
						applog(LOG_NOTICE,"reinit fail, restart cgminer!!!");
        ...
        if(diff.tv_sec > 120)
        {
            ...
            applog_e(status_error,LOG_DEBUG, "%s: We have lost internet for %d seconds, so don't send work to hashboard anymore", __FUNCTION__, diff.tv_sec);
        }
        else
        {
    ...
    while(1)
    {
        ...
        for(which_chain = 0; which_chain < BITMAIN_MAX_CHAIN_NUM; which_chain++)
        {
            if(dev.chain_exist[which_chain])
            {
                ...
                sprintf(displayed_rate[which_chain],"%.2f",each_chain_h_avg[which_chain]/1000000000/10);
        ...
        */
        ...
        if(reg_value_num > 0)
        {
            for(j = 0; j < reg_value_num; j++)
            {
                ...
                {
                    applog(LOG_DEBUG,"%s: the CHIP_ADDR reg_value = 0x%08x @chain%d", __FUNCTION__, reg_data, chain_id);
                    if(update_asic_num[chain_id])
                    {
                ...
                {
                    applog(LOG_NOTICE,"%s: the Chip%d PLL_PARAMETER reg_value = 0x%08x @chain%d", __FUNCTION__, which_chip, reg_data, chain_id);
                }

                if ( reg_addr == MISC_CONTROL )
                {
                    applog(LOG_NOTICE,"%s: the Chip%d MISC_CONTROL reg_value = 0x%08x @chain%d", __FUNCTION__, which_chip, reg_data, chain_id);
                }

                if ( reg_addr == TICK_MASK)
                {
                    applog(LOG_NOTICE,"%s: the Chip%d TICK_MASK reg_value = 0x%08x @chain%d", __FUNCTION__, chip_addr/dev.addrInterval, reg_data, chain_id);
                }
    ...
    while(1 && !reiniting[chainid])
    {
        ...
        more_work:
            ...
            */
            ...
            sia_write(info->dev_fd[chainid], (uint8_t *)&workdata, WORK_INPUT_LENGTH_WITH_CRC);
            // cg_runlock(&info->update_lock);
...
void *get_asic_response(void* arg)
{
    ...
    unsigned char receive_buf[ASIC_RETURN_DATA_LENGTH*90] = {0};    // used to receive data
    ...
    {
        cgsleep_us(500);//dev.timeout);
        len = sia_read(fd, receive_buf, ASIC_RETURN_DATA_LENGTH*90);
        for(i = 0; i < len; i++)
        {
