...
char g_miner_version[256] = {0};        // store cgminer version number
...
extern int opt_bitmain_sia_freq;
...
void sia_regen_hash ( struct work* work )
{
...
static struct opt_table opt_config_table[] =
{
    ...
    OPT_WITH_ARG("--bitmain-freq",
                 set_int_0_to_9999,opt_show_intval, &opt_bitmain_DASH_freq,
                 "Set frequency"),
    OPT_WITH_ARG("--bitmain-core-temp",
                 set_int_0_to_100,opt_show_intval, &opt_bitmain_DASH_core_temp,
                 "Select core temp"),
...
#ifdef USE_BITMAIN_A3
    OPT_WITH_ARG("--bitmain-freq",
                 set_int_0_to_9999,opt_show_intval, &opt_bitmain_sia_freq,
                 "Set frequency"),
    OPT_WITH_ARG("--bitmain-core-temp",
                 set_int_0_to_100,opt_show_intval, &opt_bitmain_sia_core_temp,
                 "Select core temp"),
    OPT_WITH_ARG("--bitmain-voltage",
...
#endif
        {
            ...
            else
            {
                snprintf(s, sizeof(s),
                         ...
                         pool->rpc_user, work->job_id, header, sshare->id);
                free(header);
...
void rebuild_nonce(struct work *work, uint32_t nonce)
{
...
void update_work_stats(struct thr_info *thr, struct work *work)
{
    ...
    if (unlikely(work->share_diff >= test_diff))
    {
        ...
        found_blocks++;
        work->mandatory = true;
        applog(LOG_DEBUG, "Found block for pool %d!", work->pool->pool_no);
    }
#endif
    mutex_lock(&stats_lock);
...
extern void doOneGoldenData(const char *testDir, int testIndex);
